...
import { random2, random1, intersect } from "./utils";
...
class Roadmap {
    ...
    init() {
        this.blankDistinct = new Array<vec2>();
        for(let i = 0; i < this.terrain.width; i ++) {
            for(let j = 0; j < this.terrain.height; j++) {
                if (this.terrain.getColor(i, j, 0) > 0.5) {
                    this.blankDistinct.push(vec2.fromValues(i, j));
                }
            }
        }
    }
    ...
    generateStreet() {
        this.seed = random1(this.seed);
        let idx = Math.floor(this.seed * this.blankDistinct.length);
        let pos = vec2.fromValues(this.blankDistinct[idx][0], this.blankDistinct[idx][1]);
        let dir = vec2.create();
